...
- 首次使用后，面板隐藏时继续缓存窗口尺寸相关的渲染器，避免重复初始化
  Direct2D 和文字资源；隐藏期间不重绘，退出或设备丢失时仍会释放资源。
...
0.5.8 仍是事件驱动的原生 Win32 程序，在设定的刷新定时器之间不做持续
动画或轮询。面板首次绘制后会缓存 Direct2D 和文字资源；隐藏窗口保持静止，
不进行后台绘制，从而降低重复展开延迟。Windows 仍可按需回收共享页或驻留页；
具体内存会随 Windows 版本、DPI、显卡驱动以及近期使用情况变化。
